...
The upstream commit also drops abbrev.c from find-gc-source-files and
renames x11term.c and x11fns.c to xterm.c and xfns.c.  22.3 still has
src/abbrev.c, so that one is kept; it does not have x11term.c or x11fns.c
but does have xterm.c and xfns.c, so that rename is taken.
