...
for p in $LIST; do
	...
	*) echo "  --- $p  受け付けない ($acc)"; skip=$((skip+1)); continue ;;
	...
	if [ $prc -eq 0 ]; then
		...
		# grep -c は 0 件でも "0" を出して exit 1 する。|| echo 0 で受けると
		# 値が "0\n0" の二行になるので、status だけ捨てる。file が無い
		# ときだけ空になるので、そこを 0 に寄せる。
		w=$(grep -cE '^Warning:|^In .*:$' "/tmp/$(basename $p).log" 2>/dev/null || true)
		[ -n "$w" ] || w=0
