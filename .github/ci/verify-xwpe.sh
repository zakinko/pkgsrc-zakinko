...
# \x1b を書かないこと。NetBSD 9.4 の sed はそれを escape と解さないので、
# 何も落ちないまま素通りする。落ちていない画面に menu を探すと、たまたま
# 属性の変わり目に当たらなかった語だけが見つかり、Edit と Block と Options と
# Window が「出ていない」と出た。xwpe は動いていて、検査のほうが移植性を
# 欠いていた。escape そのものを printf で作って渡す。
_ESC=$(printf '\033')
_strip() {
	LC_ALL=C sed -e "s/${_ESC}\[[0-9;?]*[a-zA-Z]//g" -e "s/${_ESC}[()][A-Z0-9]//g" \
