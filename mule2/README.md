...
当てている 111 ファイルのうち、30.1 まで現存するのは 55、20.1 の時点で既に無い
(mule 独自のものと生成済み info) のが 33、途中で移動または廃止されたのが 25。
...
| `patch-lib-src_b2m.c` | `lib-src/b2m.c` | gets(3) をやめる。標準ヘッダを足し、strtok の手書き宣言を外す | 読了 | 24.1 で廃止 | **20.1 の時点で独自の readline に置き換え済み**。24.1 で b2m ごと廃止 |
