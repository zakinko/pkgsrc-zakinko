# $NetBSD$
#
# editors/emacs/modules.mk の写し。上流 rev 1.40 (2026-08-25 17:33:12 wiz)
# から取った。
#
# 上流はその 1.40 で emacs26/27/28 を、それ以前に emacs22 から emacs25 を
# 消したので、下の _EMACS_VERSIONS_ALL と _EMACS_PKGDIR_MAP からも落ちて
# いる。どちらも = 代入で、しかも bsd.prefs.mk を読んだ後にあるため、
# mk.conf からもパッケージ側からも足せない。EMACS_VERSIONS_ACCEPTED に
# 書くだけでは版の検査は通るが、_EMACS_PKGDIR が空のままになり、その先の
#
#	.include "${_EMACS_PKGDIR}/version.mk"
#
# で止まる。zakinko が引き取った emacs22 から emacs28 を EMACS_TYPE として
# 指すには、この二つの表を持つファイルごと写すほかない。/usr/pkgsrc の中を
# 書き換えないという決まりもある。
#
# 上流との差はその二つの表と、それを説明している注釈だけにしてある。ほかは
# 1.40 のままなので、上流が動いたら diff で追える。
#
# 使う側は
#
...
# を
#
#	.include "../../zakinko/emacs/modules.mk"
#
# に替える。上流の版 (emacs20/21/29/30/31, xemacs) はそのまま指せるので、
# 替えたことで狭くなるところは無い。
#
...
_EMACS_VERSIONS_ALL= \
	...
	emacs22 emacs22nox emacs23 emacs23nox emacs24 emacs24nox \
	emacs25 emacs25nox emacs26 emacs26nox emacs27 emacs27nox \
	emacs28 emacs28nox \
...
_EMACS_PKGDIR_MAP= \
	...
	emacs22@../../zakinko/emacs22 \
	emacs22nox@../../zakinko/emacs22-nox11 \
	emacs23@../../zakinko/emacs23 \
	emacs23nox@../../zakinko/emacs23-nox11 \
	emacs24@../../zakinko/emacs24 \
	emacs24nox@../../zakinko/emacs24-nox11 \
	emacs25@../../zakinko/emacs25 \
	emacs25nox@../../zakinko/emacs25-nox11 \
