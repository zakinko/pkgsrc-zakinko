...
The Linux half of the packet helpers.

New file.  The second half of packet.c, moved rather than rewritten.  The one
change is that the link address is copied into a real sockaddr_ll before being
handed to sendmsg(), rather than passing n-dhcp4's wider type through.
...
+ *
+ * AF_PACKET/SOCK_DGRAM: the kernel prepends the link header from the
+ * sockaddr_ll handed to sendmsg(), and a read returns one packet with the link
+ * header already stripped.  The code below is what used to be the second half
+ * of packet.c, moved rather than rewritten.
+ */
+
+#include <assert.h>
+#include <c-stdaux.h>
