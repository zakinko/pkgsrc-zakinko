...
The Linux half of the socket helpers.

New file, and the same code that was in util/socket.c.  Binding a socket to an
interface rather than to an address is a Linux facility: SO_BINDTOIFINDEX where
it exists, SO_BINDTODEVICE before that.

util/socket.c is left in the tree; nothing lists it as a source any more.
...
+ */
+static int socket_SIOCGIFNAME(int socket, int ifindex, char (*ifnamep)[IFNAMSIZ]) {
