...
lic={'GPL-3+':'gnu-gpl-v3','GPL-3':'gnu-gpl-v3','GPL-2+':'gnu-gpl-v2','GPL-2':'gnu-gpl-v2','MIT':'mit','BSD':'modified-bsd','BSD-2':'2-clause-bsd','Apache-2.0':'apache-2.0','Unlicense':'unlicense','public-domain':'public-domain','MPL-2.0':'mpl-2.0','ISC':'isc','ZLIB':'zlib','WTFPL-2':'wtfpl\t# licenses/wtfpl comes with this set','mpg123-el':'mpg123-el-license\t# licenses/mpg123-el-license comes with this set','GPL-1+':'gnu-gpl-v1 OR gnu-gpl-v2 OR gnu-gpl-v3','LGPL-3+':'gnu-lgpl-v3','LGPL-2.1+':'gnu-lgpl-v2.1','CC0-1.0':'cc0-1.0-universal','||':'mit'}
...
for n,r in recs.items():
    ...
    L+=["",f"MAINTAINER=\tpkgsrc-users@NetBSD.org",f"HOMEPAGE=\t{r['home']}",f"COMMENT=\t{re.sub(r'^(A|An|The) ','',r['desc'][:70].rstrip('.')).replace('\\$','$').replace('$','$$')}",f"LICENSE=\t{r['lic']}","","EMACS_VERSIONS_ACCEPTED=\temacs29 emacs29nox emacs30 emacs30nox emacs31 emacs31nox"]
    lp=[]
