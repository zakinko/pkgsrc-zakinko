...
Same change on the Emacs side of the CLISP dialect, plus the prompt.

ilisp-reset sends (sys::debug-unwind), which fails for the reason in patch-ad;
(sys::debug-quit nil) is what unwinds to the top level in this CLISP.

comint-prompt-regexp does not match the prompt CLISP actually prints, so ILISP
never recognises that the inferior Lisp is ready.
