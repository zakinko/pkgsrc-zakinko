...
build here -- emacs20 20.7, 21.4, 22.3, 23.4, 24.5, 25.3, 26.3, 27.2,
28.2, 29.4, 30.2, 31.1, and XEmacs 21.4.25 and 21.5-b36 -- by loading it
and asking for `alist-get`, `when-let` and `delete-dups` afterwards.
...
| devel/elisp-compat | – | 20260924 | **new** (`new/elisp-compat/`): one file, elisp-compat.el, defining -- only when the running Emacs does not have it -- what elisp written since assumes.  It replaces three packages that were separate before: the Emacs 20 set, an Emacs 21-to-23 set and an XEmacs set.  Counted, the Emacs 20 file already held 56 of the definitions and the others added three between them (`alist-get`, `seq-filter`, `delete-dups`), and it is written in the syntax Emacs 20's reader accepts, so one file can serve them all.  What it defines: hash tables (over alists), replace-regexp-in-string, propertize, nbutlast, subr-x's strings, setq-local, with-eval-after-load, prog-mode, syntax-ppss, syntax-propertize (rules become a function run from post-command-hook and, on Emacs 20, also font-lock-syntactic-keywords), obarray-make, set-process-query-on-exit-flag, dolist, cl's remove, the WHEN argument make-obsolete-variable grew in Emacs 23, alist-get, seq-filter, delete-dups, and a regexp translator for shy groups, `\\{n,m\\}`, `[:alpha:]` and `\\_<` that remaps the match data afterwards.  Two things XEmacs needed: `regexp-opt` does not exist on 21.4 (21.5 has it), so the require that reaches for it is wrapped in a condition-case rather than left bare, and `eval` takes one argument there, so it is wrapped to accept and ignore Emacs 24's LEXICAL -- only on XEmacs, since changing `eval` on GNU Emacs 20 to 23 has not been measured.  Loaded on all fourteen: emacs20 20.7 through emacs31 31.1, xemacs214 21.4.25 and xemacs215 21.5-b36, with alist-get, when-let and delete-dups present afterwards on each; built for emacs20 and emacs30nox.  `provide` gives elisp-compat and also emacs-compat, e20-compat, xemacs-compat and subr-x, so an existing require keeps working.  Not called emacs-compat: `wip/emacs-compat` is already GNU ELPA's compat, a different library for Emacs 25.1 and later (its compat-29.el cannot even be read by the old readers -- "Invalid escape character syntax" on Emacs 20 and 21, "Weird old-backquote syntax" on XEmacs 21.4), and under the xemacs prefix emacs-compat would come out as xemacs-emacs-compat.  nxml-mode, graphviz-dot-mode, verilog-mode and tamago-tsunagi use it |
...
| devel/bazel | 6.4.0 | 9.2.0 | not an Emacs package, but mozc's: bazel is a TOOL_DEPENDS of the mozc packages and of nothing else in the tree, no package reaches it through buildlink3.mk, and mozc 3.34 will not build against 6.4.0.  Upstream bazel does not know about NetBSD -- the enum in OS.java lists FreeBSD and OpenBSD only -- so 28 patches add it.  Eleven files are new and the `CONFLICTS+= bazel-[0-9]*` that the out-of-tree copy carried is dropped, because as devel/bazel it would match its own name.  Applied to a checkout from today with no fuzz and no offset |
| inputmethod/mozc-server, mozc-elisp, mozc-renderer, mozc-tool | 2.29.5268.102 | 3.34.6239 | **the four go together**: mozc-elisp, mozc-renderer and mozc-tool read mozc-server's `Makefile.common` and `options.mk`, so applying one without the others does not build.  3.34 builds with gyp on every architecture instead of needing bazel, which is why the four can be updated at all here -- the 2.29 set wanted bazel on NetBSD x86_64 and aarch64, and bazel's bootstrap wants a JVM.  `options.mk` takes `bazel` and `gyp` as options with gyp suggested, and where bazel cannot build -- it needs a JVM to bootstrap, and its own singlejar refuses 32-bit, which leaves NetBSD x86_64 and aarch64 -- asking for bazel sets MOZC_BUILDER back to gyp and says so through WARNINGS rather than stopping the build.  The 2.29 set in the tree had the gyp option drop out on NetBSD x86_64 and aarch64 instead.  Sixteen new patches arrive with mozc-server and seventeen files go empty, which patch cannot remove -- they need `cvs rm`: `mozc-elisp/MESSAGE`, and in `mozc-server/patches/`: `patch-WORKSPACE.bazel`, `patch-base_logging.cc`, `patch-base_system__util__test.cc`, `patch-bazel_pkg__config__repository.bzl`, `patch-config.bzl`, `patch-config_stats__config__util__test.cc`, `patch-dictionary_BUILD.bazel`, `patch-gui_config__dialog_config__dialog.cc`, `patch-gui_config__dialog_keybinding__editor.cc`, `patch-gui_config__dialog_keymap__editor.cc`, `patch-gui_dictionary__tool_dictionary__tool.cc`, `patch-gui_word__register__dialog_word__register__dialog.cc`, `patch-session_session__handler__scenario__test.cc`, `patch-session_session__test.cc`, `patch-third__party_abseil-cpp_absl_base_internal_raw__logging.cc` and `patch-third__party_protobuf_toolchain_cc__toolchain__config.bzl`.  Applied to a checkout from today with no fuzz and no offset, and the result compared file by file against the packages this came from |
